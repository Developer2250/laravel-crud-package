...
@section('content')
    <div class="mt-4">
        ...
        <form action="{{ route('books.update', $item->id) }}" method="POST" id="edit-form">
            @csrf
            @method('PUT')
                <div class="mb-3">
    ...
    <div class="mb-3">
        <label for="author_id" class="form-label">{{ __('labels.author_id') }} <span class="required">*</span></label>
            ...
            @foreach($authors as $relItem)
                <option value="{{ $relItem->id }}" {{ old('author_id', $item->author_id) == $relItem->id ? 'selected' : '' }}>
                    {{ $relItem->full_name ?? $relItem->first_name ?? $relItem->name ?? 'N/A' }}
                </option>
            @endforeach
        </select>
        @error('author_id')
    ...
    <div class="mb-3">
        <label for="published_date" class="form-label">{{ __('labels.published_date') }} <span class="required">*</span></label>
                <input type="date" name="published_date" id="published_date" class="form-control @error('published_date') is-invalid @enderror" placeholder="Enter {{ __('labels.published_date') }}" value={{ old('published_date', optional($item->published_date)->format('Y-m-d')) }}>
    ...
    <div class="mb-3">
        <label for="isbn" class="form-label">{{ __('labels.isbn') }} <span class="required">*</span></label>
                <input type="text" name="isbn" id="isbn" class="form-control @error('isbn') is-invalid @enderror" placeholder="Enter {{ __('labels.isbn') }}" value={{ old('isbn', $item->isbn) }}>
        @error('isbn')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="mb-3">
        <label for="summary" class="form-label">{{ __('labels.summary') }} <span class="required">*</span></label>
                <textarea name="summary" id="summary" class="form-control @error('summary') is-invalid @enderror" placeholder="Enter {{ __('labels.summary') }}" rows="4" cols="50">{{ old('summary', $item->summary) }}</textarea>
        @error('summary')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
